...
APP_VERSION = "3.6.0"       # wird in der Web-Oberflaeche und im Log angezeigt
...
TIMER_SLOTS = 10            # so viele Zeitplaene bietet die Oberflaeche an
WEEKDAYS = ["Mo", "Di", "Mi", "Do", "Fr", "Sa", "So"]   # Index = date.weekday()

# Vorgabe: Autorecord 1 laeuft werktags von 08:45 bis 18:30
# Die ersten drei sind vorbelegt, der Rest steht bereit und ist ausgeschaltet.
DEFAULT_TIMERS = [
...
DEFAULT_SETTINGS = {
    ...
    "timer_count": 1,               # sichtbare/aktive Zeitplaene: 1 bis TIMER_SLOTS
...
_timer_state = {
    ...
    "stop_pending": False,  # Fenster ist aus, die Aufnahme laeuft noch
    "warned_orphan": False, # Hinweis auf eine herrenlose Aufnahme kam schon
...
def run_timer(now=None):
    ...
    if not cfg["timer_enabled"]:
        ...
        if stale or _timer_state["window"] is not None:
            _timer_state.update({"window": None, "index": None,
                                 "next_try": 0.0, "stop_pending": False})
    ...
    # Ausserhalb aller Fenster: Der Stopp bleibt offen, bis er greift. Frueher
    # galt dafuer eine Frist von fuenf Minuten - ein WLAN-Aussetzer genau zum
    # Feierabend reichte, und die Aufnahme lief die ganze Nacht weiter.
    # Eine spaeter von Hand gestartete Aufnahme hebt den Stopp auf (handle_job).
    if recording and _timer_state["stop_pending"]:
        _timer_state["next_try"] = monotonic + TIMER_RETRY_S
        do_stop(manual=False)
        return
    if not recording:
        _timer_state["stop_pending"] = False
        _timer_state["warned_orphan"] = False
        return

    # Es laeuft eine Aufnahme, zu der dieser Dienst kein Fenster kennt - etwa
    # weil der Rechner neu gestartet wurde, nachdem das Fenster abgelaufen war.
    # Automatisch gestoppt wird sie nicht, denn sie kann von Hand gestartet
    # worden sein. Stillschweigend uebergehen sollte man sie aber auch nicht.
    if not _timer_state["warned_orphan"]:
        _timer_state["warned_orphan"] = True
        log("Es laeuft eine Aufnahme ausserhalb aller Zeitfenster. Dieser Dienst "
            "hat sie nicht gestartet und stoppt sie nicht von selbst.", "warn")

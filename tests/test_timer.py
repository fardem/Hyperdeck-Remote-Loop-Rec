...
def test_state_machine_missed_stop_is_retried_then_given_up():
    with Harness() as h:
        h.reset()
        hc.run_timer(at(0, 9, 0))
        ...
        hc.set_state(status="record")
        h.clock.t += hc.TIMER_RETRY_S + 1
        hc.run_timer(at(0, 18, 31));  assert h.calls == ["record", "stop", "stop"]
        hc.set_state(status="record")
        h.clock.t += hc.TIMER_STOP_GRACE_S + 100
        hc.run_timer(at(0, 18, 40));  assert h.calls == ["record", "stop", "stop"]
